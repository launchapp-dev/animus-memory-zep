...
export class MemoryStoreHandler {
  ...
  async get(req: GetMemoryRequest): Promise<GetMemoryResponse> {
    ...
    // Search-based fallback — Zep has no native key-value get. We query
    // `scope: 'episodes'` with the key as the search string and post-filter
    // for an exact `metadata.key` match. The capability declaration tells the
    // daemon this is not O(1).
    //
    // TODO(codex-p2): For scopes with >limit episodes (or where the semantic
    // reranker buries an exact key past the first page) memory/get may return
    // `found: false` for a key that actually exists. v0.5 ships this as a
    // documented limitation — the capability flag `native_key_get: false`
    // tells the daemon not to rely on `get` for completeness. A robust fix
    // requires either a Zep metadata-filter query (not exposed by the v3.x
    // SDK at time of writing) or a paginated episodes scan. Tracked for v0.6.
    let results;
    try {
      results = await this.deps.client.search({
        ...
        query: req.key,
        scope: 'episodes',
        // Cap matches Zep's hard limit; a future iteration may page beyond.
 ...
 * fall back to reading episode metadata.
 *
 * TODO(codex-p2): the `__` delimiter can in principle appear inside a
 * normalized segment (e.g. an agent_id of `a__b` round-trips through
 * `normalize()` unchanged, producing `proj_p__agent_a__b`, which this parser
 * mis-splits into a 4-part array and returns `null` for). v0.5 ships this as
 * a known limitation — `memory/list_scopes` will simply omit affected scopes
 * rather than mis-report them, and the un-normalized originals remain
 * available in episode metadata. A robust fix requires either escaping the
 * delimiter during graphId construction (breaking change for any stored
 * graphs) or fetching one episode per graph in `list_scopes` to read the
 * `*_raw` metadata fields. Tracked for v0.6.
 */
